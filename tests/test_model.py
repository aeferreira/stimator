...
def test_set_reaction1b():
    ...
    m.set_reaction('v3', "2C<=>D", 2.0)
    m.set_reaction('v4', "0C+2D->E", 2.0)
    _ = m.reactions.v1.name
    _ = m.reactions.v1.reagents
    _ = m.reactions.v1.products
    _ = m.reactions.v1.stoichiometry
    _ = m.reactions.v1.stoichiometry_string
    _ = m.reactions.v3.stoichiometry
...
def test_par_from_rates1():
    ...
    m = Model("My first model")
    m.set_reaction('v1', "A->B", " p2*A/(p1+A)-B ",
                   pars=(('p1', 4.0), ('p2', 2)))
