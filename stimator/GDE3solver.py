...
class GDE3Solver(DESolver):
    ...
    def __init__(self, models,
        ...
        # generate list of pairs of model indexes.
        # Each pair corresponds to a different comparison
        self.model_indexes = []
    ...
    def computeGeneration(self):
        ...
        if self.generation == 0:
            ...
            for p in range(self.pop_size):
                ...
                # Reject those out-of-bounds or repeated
            ...
            for ndf in nondominated_waves:
                ...
                if len(self.pop) < self.pop_size and excess >= 0:
                    ...
                    # to exactly complete pop to pop_size
            ...
            print('generations with no improvement:', self.gen_no_improv)

            timeElapsed = time() - time0

            print(f"\nGeneration {self.generation:d} finished, took {timeElapsed:6.3f} s")
            print(f'{timeElapsed1:6.3f} s generating new pop')
            print(f'{timeElapsed2:6.3f} s in non-dominant sorting')
            self.gen_times.append(timeElapsed)
            if self.dump_generations is not None:
                if self.generation in self.dump_generations:
                    print(self.generation_string(self.generation),
                          file=self.dumpfile)
    ...
    def finalize(self):
        ...
        if self.dump_generations is not None:
            if self.generation-1 not in self.dump_generations:
                print(self.generation_string(self.generation-1),
                      file=self.dumpfile)
            self.dumpfile.close()
