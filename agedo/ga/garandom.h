// $Header: //194.117.40.12/c\044/Documents\040and\040Settings/CVSNT/Repositories/Bioquimica\040Teorica/AGEDO/Source/ga/garandom.h,v 1.1 2003/09/05 16:46:17 Administrator Exp $
/* ----------------------------------------------------------------------------
  random.h
  mbwall 29jun95
...
#if defined(USE_RAN1) || defined(USE_RAN2) || defined(USE_RAN3) || defined(USE_RAN_MT)
...
namespace Mersenne_Twister {
	void init_genrand(unsigned long seed=1);
	double genrand_real1(void);
}
#define _GA_RND             Mersenne_Twister::genrand_real1
#define _GA_RND_SEED        Mersenne_Twister::init_genrand
...
inline float GARandomFloat(){ return float(_GA_RND()); }
inline float GARandomFloat(float low, float high){
  float val=high-low; val*=float(_GA_RND()); return val+low;
}
