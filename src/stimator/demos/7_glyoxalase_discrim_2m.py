...
def compute(obj):
    ...
    print(f'Metrics used: {obj}\n')
    print(f'observed variables: {observed}\n')
    print('initial values to optimize:')
    for name, min_v, max_v in initial_opt:
        print(name, f'in [{min_v:g}, {max_v:g}]')
    ...
    with open('glos_2m_{}_final_pop.txt'.format(obj), 'w') as f:
        ...
        for s, o in zip(solver.pop, solver.population_energies):
            sstr = ' '.join([f"{i:6.4g}" for i in s])
            ostr = ' '.join([f"{i:6.4g}" for i in o])
            print(f'{sstr} ----> {ostr}')
            print(f'{sstr} {ostr}', file=f)
